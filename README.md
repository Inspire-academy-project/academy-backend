...
원생은 로그인 계정 없이 등록된다. 계정은 나중에 **가입 코드**로 잇는다(아래 참고).
...
| 메서드 | 경로             | 권한   | 설명                              |
| ------ | ---------------- | ------ | --------------------------------- |
| POST   | `/invite/verify` | 공개   | 가입 코드가 누구 것인지 확인      |
| POST   | `/register`      | 공개   | **가입 코드로만** 가입            |
...
| GET    | `/:id`         | ADMIN, TEACHER | 상세 + 최근 출결 30건/납부 24건 + 계정 연결 상태 |
| POST   | `/`            | ADMIN, TEACHER | 원생 등록                                      |
| POST   | `/:id/invite`  | ADMIN, TEACHER | 가입 코드 발급                                 |
| PATCH  | `/:id`         | ADMIN, TEACHER | 좌석번호·연락처·재원 여부 수정                 |

### 원생과 계정 잇기

원생은 원장이 명부를 보고 등록한다. 학생이 스스로 가입하면 **같은 사람이 둘로 갈라진다** —
명부의 홍길동과 가입해서 생긴 홍길동이 다른 사람이 되어, 출결·납부는 앞쪽에 쌓이는데
학생은 뒤쪽으로 로그인하니 자기 기록이 안 보인다.

그래서 가입은 **원장이 발급한 코드로만** 된다.

```
원장  →  원생 상세에서 코드 생성  →  학생에게 전달
학생  →  코드 + 이메일 + 비밀번호로 가입  →  기존 원생에 연결됨
```

```bash
# 1. 원장이 발급 — 코드 원문은 이 응답에만 실린다
POST /api/students/2/invite
→ { "studentId": 2, "name": "홍길동", "code": "A7K2-9QX4", "expiresAt": "..." }

# 2. 학생이 확인 — 남의 코드로 가입하는 사고를 막는다
POST /api/auth/invite/verify   { "code": "A7K2-9QX4" }
→ { "name": "홍길동", "expiresAt": "..." }

# 3. 학생이 가입 — Student를 새로 만들지 않고 기존 원생에 잇는다
POST /api/auth/register        { "code": "...", "email": "...", "password": "..." }
```

알아둘 점

- **코드는 해시로 저장한다.** 원문은 발급하는 순간에만 볼 수 있다. 잃어버리면 다시 발급한다
- **다시 발급하면 이전 코드는 무효**가 된다. 여러 개가 살아 있으면 어느 걸 전달했는지 몰라 회수할 수 없다
- 유효기간 **7일**, **1회용**
- 소문자로 치거나 하이픈을 빼먹어도 같은 코드로 본다 (`a7k29qx4` = `A7K2-9QX4`)
- 헷갈리는 글자 `I·O·0·1`은 코드에 쓰지 않는다
- **출결번호로는 잇지 않는다.** 출결번호는 패드에서 공개적으로 눌리는 값이라 학생끼리
  서로 안다. 그걸로 가입시키면 남의 계정을 만들 수 있다
- **퇴원하면 로그인이 막힌다**(403). 기록은 원장·강사 화면에 그대로 남는다.
  퇴원생에게는 코드도 발급되지 않는다
...
| GET    | `/?date=YYYY-MM-DD`   | ADMIN, TEACHER | **그날 다니던** 명단 + 출결 상태   |
...
원생은 미리 등록해 둘 수 있다. 9월 1일 등록 예정인 학생을 지금 넣어도
**등록일 전에는 출결 명단에 나오지 않고 패드에서도 찍히지 않는다.**
등원할 때까지 매일 미입력으로 남는 걸 막기 위함이다.

퇴원생도 같은 규칙이다. 지금 안 다닌다고 지난 8월 명단에서 빼면 그때 출결을
볼 수 없으므로, 퇴원일까지는 명단에 남는다.

```
그날 명단 = 등록일 <= 조회날짜  AND  (재원 중 이거나 퇴원일 >= 조회날짜)
```

학원비도 같은 규칙으로 동작한다(`tuition.ts`). 미래 등록생에게는 청구서를 만들지 않고,
주기 중간에 등록했으면 일할계산한다.

패드에서 등록일 전 번호를 찍으면 `403 아직 등원 시작일이 아닙니다.` 가 나온다.
번호를 잘못 눌렀을 때(`404`)와 문구를 나눈 이유는, 뭉뚱그리면 학생이 자기 번호가
틀렸다고 오해하고 계속 다시 누르기 때문이다.
